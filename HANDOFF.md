...
- **Live:** https://cart-a.live  (and `carta-beta-eosin.vercel.app`)
...
7. **Facility switcher** in sidebar — dropdown listing accessible facilities (helpful for cost controllers).

8. **Sales bulk paste** in Matrix — paste from Excel/clipboard into table.

9. **Cost bulk import (.xlsx)** — was in Barceló v4.6 (native DecompressionStream + ZIP central dir walk). Port to `cost.html`.

10. **Allergen icons** — visual chips with icon + label (currently text-only).

11. **Recipe → menu cluster assignment** — currently `menu_clusters` table exists but no UI to group recipes into menus (Breakfast / Lunch / Dinner).

12. **Audit log UI** — `audit_log` table exists but no view; build a timeline page (`app/studio/audit.html`).

13. **Workspace settings page** — currently only Manage modal in selector; add a dedicated settings view inside the studio for currency/VAT/GP/Q-factor + rename + transfer ownership.

### Compliance / Ops

14. **KVKK VERBİS registration** — required before serving paid customers in TR
15. **Data export per workspace** — GDPR right to portability
16. **Data deletion request** — currently delete cascades but no "purge user data" cross-workspace flow
17. **Audit log retention policy** — define how long we keep `audit_log` rows
